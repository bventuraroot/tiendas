...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Amilasa Lipasa' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.amilasa_lipasa.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'amilasa', 'label' => 'Amilasa', 'unidad' => 'U/L'],
            ['key' => 'lipasa', 'label' => 'Lipasa', 'unidad' => 'U/L'],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">ENZIMAS</td>
            </tr>
            @foreach($params as $p)
                @php
