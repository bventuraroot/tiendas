...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Cortisol A.M' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
            ...
            @php
                ...
                if (!$valorReferencia) {
                    if (isset($valoresReferencia['valores_referencia'])) {
                        ...
                        $valorReferencia = '';
                        if (isset($valRef['am'])) {
                            $valorReferencia .= 'A.M: ' . ($valRef['am']['rango'] ?? '5.0 a 25.0') . "\n";
                        }
                        if (isset($valRef['nota'])) {
                            $valorReferencia .= "\n" . $valRef['nota'];
                        }
                    } else {
                        $valorReferencia = $exam->valores_referencia ?? 'A.M: 5.0 a 25.0 ug/dL';
                    }
                }
            @endphp

            <tr>
                <td class="exam-name">Cortisol A.M:</td>
