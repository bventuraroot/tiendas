...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Bilirrubina' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.bilirrubina.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'total', 'label' => 'Bilirrubina Total', 'unidad' => 'mg/dL'],
            ['key' => 'directa', 'label' => 'Bilirrubina Directa', 'unidad' => 'mg/dL'],
            ['key' => 'indirecta', 'label' => 'Bilirrubina Indirecta', 'unidad' => 'mg/dL'],
        ];
    @endphp
