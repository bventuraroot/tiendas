...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Anti-estreptolisina O (ASO)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'SEROLOGIA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'UI/L');
            @endphp
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['adultos']))
                            <div>{{ $valoresReferencia['adultos']['label'] ?? 'Adultos' }}: {{ $valoresReferencia['adultos']['rango'] ?? 'Hasta 200 UI/L' }}</div>
                        @endif
                        @if(isset($valoresReferencia['ninos']))
                            <div>{{ $valoresReferencia['ninos']['label'] ?? 'Niños' }}: {{ $valoresReferencia['ninos']['rango'] ?? 'Hasta 150 UI/L' }}</div>
                        @endif
                    @else
                        <div>Adultos: Hasta 200 UI/L</div>
                        <div>Niños: Hasta 150 UI/L</div>
                    @endif
                </td>
            </tr>
        </tbody>
    ...
    <div class="instructions-section">
        <div class="instructions-title">(SI APARECE AGLUTINACION HACER DILUCION)</div>
        <div class="instructions-title">PARA REPORTE CON DILUCION HACER LA SIGUIENTE FORMULA</div>
        <div class="formula-example">
            EJEMPLO 1/16<br>
            CONCENTRACION DE FR = 8X16= **128 UI/L**
        </div>
    </div>
