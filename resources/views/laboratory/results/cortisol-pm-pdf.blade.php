...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Cortisol P.M' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'HORMONAS' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unidad = $result ? ($result->unidad_medida ?? 'ug/dL') : 'ug/dL';
                $valorReferencia = $result ? $result->valor_referencia : ($valoresReferencia['valores_referencia'] ?? '');

                // Si no hay valor de referencia en el resultado, usar el del examen o uno por defecto
                if (!$valorReferencia) {
                    if (isset($valoresReferencia['valores_referencia'])) {
                        $valRef = $valoresReferencia['valores_referencia'];
                        $valorReferencia = '';
                        if (isset($valRef['pm'])) {
                            $valorReferencia .= 'P.M: ' . ($valRef['pm']['rango'] ?? '3.0 a 16.0') . "\n";
                        }
                        if (isset($valRef['nota'])) {
                            $valorReferencia .= "\n" . $valRef['nota'];
                        }
                    } else {
                        $valorReferencia = $exam->valores_referencia ?? 'P.M: 3.0 a 16.0 ug/dL';
                    }
                }
            @endphp

            <tr>
                <td class="exam-name">Cortisol P.M:</td>
