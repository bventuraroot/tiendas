...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Glucosa' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
    ...
    <table class="results-table">
        ...
        <tbody>
            ...
            @php
                ...
                $resultadoAyunas = $results->firstWhere('parametro', 'Glucosa en Ayunas');
                $resultadoPostPrandial = $results->firstWhere('parametro', 'Glucosa Post-Prandial');
                
                $unidad = $valoresReferencia['unidad_medida'] ?? 'mg/dL';
            @endphp
            ...
            <tr>
                ...
                <td style="text-align: center;">
                    @if($resultadoAyunas)
                        {{ $resultadoAyunas->resultado }} {{ $unidad }}
                    @endif
                </td>
                <td class="reference-range">
                    @if($valoresReferencia && isset($valoresReferencia['ayunas']))
                        {{ $valoresReferencia['ayunas']['rango'] ?? '70 a 110 mg/dL' }}
                    @else
                        70 a 110 mg/dL
                    @endif
                </td>
            ...
            <tr>
                <td class="exam-name">Glucosa Post-Prandial:</td>
                <td style="text-align: center;">
                    @if($resultadoPostPrandial)
                        {{ $resultadoPostPrandial->resultado }} {{ $unidad }}
                    @endif
                </td>
                <td class="reference-range">
                    @if($valoresReferencia && isset($valoresReferencia['post_prandial']))
                        {{ $valoresReferencia['post_prandial']['rango'] ?? 'Hasta 140 mg/dL' }}
                    @else
                        Hasta 140 mg/dL
                    @endif
                </td>
