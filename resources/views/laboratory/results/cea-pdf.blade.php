...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'CEA (Antígeno Carcinoembriogénico)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'INMUNOLOGÍA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'ng/mL');
            @endphp
            ...
            <tr>
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['no_fumadores']))
                            <div>{{ $valoresReferencia['no_fumadores']['label'] ?? 'No Fumadores' }} {{ $valoresReferencia['no_fumadores']['rango'] ?? '0.0 a 5.0 ng/mL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['fumadores']))
                            <div>{{ $valoresReferencia['fumadores']['label'] ?? 'Fumadores' }} {{ $valoresReferencia['fumadores']['rango'] ?? '0.0 a 10.0 ng/mL' }}</div>
                        @endif
                    @else
                        <div>No Fumadores 0.0 a 5.0 ng/mL</div>
                        <div>Fumadores 0.0 a 10.0 ng/mL</div>
                    @endif
                </td>
            </tr>
        </tbody>
