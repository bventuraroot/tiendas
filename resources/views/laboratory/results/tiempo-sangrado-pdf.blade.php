...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Tiempo de Sangrado' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'HEMOSTASIA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unidad = $result ? ($result->unidad_medida ?? 'minutos') : 'minutos';
                $valorReferencia = $result ? $result->valor_referencia : ($valoresReferencia['valores_referencia']['rango'] ?? '2 a 8 minutos');
            @endphp
            
            <tr>
                <td class="exam-name">Tiempo de Sangrado:</td>
