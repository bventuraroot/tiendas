...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Pruebas Tiroideas para Niños' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        ...
        $valRef = config('lab_exam_templates.tiroideas_ninos.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 't3', 'label' => 'Triyodotironina (T3)', 'unidad' => 'Ng/mL'],
            ['key' => 't4', 'label' => 'Tiroxina (T4)', 'unidad' => 'ug/dl'],
            ['key' => 'tsh', 'label' => 'Hormona Estimulante de Tiroides (TSH)', 'unidad' => 'uUI/mL'],
        ];

        $categoria = $valoresReferencia['categoria'] ?? ($exam->category->nombre ?? 'PRUEBAS TIROIDEAS');
        $notaEdad = $valRef['nota_edad'] ?? 'Valores de referencia en niños de 1 – 6 años';
    @endphp
