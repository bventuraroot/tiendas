...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'VDRL-RPR' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'SEROLOGÍA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                $result = $results->first();
                $resultadoValue = $result ? $result->resultado : '';
                
                // Extraer campos adicionales del JSON en observaciones
                $marcaReactivo = '';
                $metodoPrueba = '';
                
                if ($result && $result->observaciones) {
                    ...
                    if (str_contains($observaciones, '__CAMPOS_ADICIONALES__')) {
                        ...
                        if (count($parts) > 1) {
                            ...
                            if ($jsonData) {
                                $marcaReactivo = $jsonData['marca_reactivo'] ?? '';
                                $metodoPrueba = $jsonData['metodo_prueba'] ?? '';
                            }
                        }
                    }
                }
            ...
            <tr>
                <td class="exam-name">{{ $exam->nombre ?? 'VDRL-RPR' }}:</td>
                <td style="text-align: center;">
                    @if($result)
                        <span class="result-bold">{{ $resultadoValue }}</span>
                    @endif
                </td>
                <td class="reference-range">
                    <!-- Rango de referencia vacío para VDRL-RPR -->
                </td>
            </tr>
            
            @if($marcaReactivo)
            <tr>
                <td class="additional-info">Marca del Reactivo:</td>
                <td style="text-align: center;">{{ $marcaReactivo }}</td>
                <td></td>
            </tr>
            @endif
            ...
            @if($metodoPrueba)
            <tr>
                <td class="additional-info">Método de Prueba:</td>
                <td style="text-align: center;">{{ $metodoPrueba }}</td>
                <td></td>
            </tr>
            @endif
        </tbody>
