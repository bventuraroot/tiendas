...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Hematocrito y Hemoglobina' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.hematocrito_hemoglobina.valores_referencia', []);
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">HEMATOLOGÍA</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    if (!$r && $resultsByParam->isNotEmpty()) {
                        ...
                        foreach ($resultsByParam as $key => $result) {
                            ...
                            if (stripos($keyLower, 'hematocrito') !== false && stripos($labelLower, 'hematocrito') !== false) {
                                $r = $result;
                                break;
                            }
                            if (stripos($keyLower, 'hemoglobina') !== false && stripos($labelLower, 'hemoglobina') !== false) {
                                $r = $result;
                                break;
                            }
                        }
