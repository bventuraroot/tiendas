...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Baciloscopía (BK esputo)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'MICROSCOPIA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $resultadoDisplay = '';
                if (stripos($resultValue, 'no se observa') !== false || stripos($resultValue, 'no observa') !== false || stripos($resultValue, 'negativo') !== false) {
                    $resultadoDisplay = 'No se Observa BAAR';
                } elseif (stripos($resultValue, 'se observa') !== false || stripos($resultValue, 'positivo') !== false) {
                    $resultadoDisplay = 'Se Observa BAAR';
                } else {
                    $resultadoDisplay = $resultValue ?: '';
                }
            @endphp
            ...
            <tr>
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['normal']))
                            {{ $valoresReferencia['normal']['rango'] ?? 'No se Observa BAAR' }}
                        @else
                            No se Observa BAAR
                        @endif
                    @else
