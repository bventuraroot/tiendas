...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Ac. Antinucleares Látex (ANA tamizaje)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
    ...
    <table class="results-table">
        ...
        <tbody>
            ...
            @php
                ...
                $resultadoDisplay = '';
                if (strtolower($resultadoValue) === 'negativo' || strtolower($resultadoValue) === 'neg') {
                    $resultadoDisplay = 'Negativo';
                } elseif (strtolower($resultadoValue) === 'positivo' || strtolower($resultadoValue) === 'pos') {
                    $resultadoDisplay = 'Positivo';
                } else {
                    $resultadoDisplay = ucfirst($resultadoValue);
                }
            @endphp
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['normal']))
                            {{ $valoresReferencia['normal']['rango'] ?? 'Negativo' }}
                        @else
                            Negativo
                        @endif
                    @else
