...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Examen General de Orina' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <!-- Tabla de Resultados (Orina Completa: grupos BIOQUIMICO / MICROSCOPICOS) -->
    @php
        $valRef = config('lab_exam_templates.general_orina.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $grupos = [
            ['grupo' => null, 'params' => ['Color', 'Aspecto']],
            ['grupo' => 'BIOQUIMICO', 'params' => ['Densidad', 'pH', 'Urobilinógeno', 'Bilirrubina', 'Cuerpos Cetónicos', 'Nitritos', 'Sangre/Hemoglobina', 'Esterasa Leucocitaria', 'Proteínas', 'Glucosa']],
            ['grupo' => 'MICROSCOPICOS', 'params' => ['Células Epiteliales', 'Filamentos Mucoides', 'Bacterias', 'Leucocitos', 'Eritrocitos', 'Cilindros', 'Cristales', 'Levaduras']],
        ];
        $keyMap = [
            'Color' => 'color', 'Aspecto' => 'aspecto', 'Densidad' => 'densidad', 'pH' => 'ph',
            'Urobilinógeno' => 'urobilinogeno', 'Bilirrubina' => 'bilirrubina', 'Cuerpos Cetónicos' => 'cuerpos_cetonicos',
            'Nitritos' => 'nitritos', 'Sangre/Hemoglobina' => 'sangre_hemoglobina', 'Esterasa Leucocitaria' => 'esterasa_leucocitaria',
            'Proteínas' => 'proteinas', 'Glucosa' => 'glucosa', 'Células Epiteliales' => 'celulas_epiteliales',
            'Filamentos Mucoides' => 'filamentos_mucoides', 'Bacterias' => 'bacterias', 'Leucocitos' => 'leucocitos',
            'Eritrocitos' => 'eritrocitos', 'Cilindros' => 'cilindros', 'Cristales' => 'cristales', 'Levaduras' => 'levaduras',
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">ORINA COMPLETA</td>
            </tr>
            @foreach($grupos as $g)
                @if($g['grupo'])
                <tr>
                    <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 0%;">{{ $g['grupo'] }}</td>
                </tr>
                @endif
                @foreach($g['params'] as $label)
                    @php
                        $r = $resultsByParam->get($label);
                        $resVal = $r ? $r->resultado : '';
                        $vr = $r && $r->valor_referencia ? $r->valor_referencia : ($valRef[$keyMap[$label] ?? '']['rango'] ?? '---');
                        ...
                        $u = $r && $r->unidad_medida ? str_replace(['μg', 'μmol', 'μL'], ['ug', 'umol', 'uL'], $r->unidad_medida) : '';
                    @endphp
                    <tr>
                        ...
                        <td class="reference-range">{{ $vr }}</td>
                    </tr>
                @endforeach
            @endforeach
        </tbody>
    ...
    <div class="validation-section">
        ...
        <div style="margin-bottom: 0%; margin-top: 0%;">Validado por: {{ $authorizedDoctorName }}</div>
        ...
        @else
        <div style="border-bottom: 1px solid #000; width: 300px; margin: 0 auto; margin-top: 0px; padding-bottom: 0%;"></div>
        @endif
    </div>
