...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'TIPEO SANGUINEO' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.tipo_sanguineo.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'grupo', 'label' => 'Grupo', 'unidad' => ''],
            ['key' => 'factor_rh', 'label' => 'Factor RH', 'unidad' => ''],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">TIPEO SANGUINEO</td>
            </tr>
            @foreach($params as $p)
                @php
