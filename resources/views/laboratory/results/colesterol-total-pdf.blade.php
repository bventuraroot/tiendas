...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'COLESTEROL TOTAL' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'QUIMICA CLINICA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'mg/dL');
            @endphp
            ...
            <tr>
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['deseable']))
                            <div>{{ $valoresReferencia['deseable']['label'] ?? 'Deseable' }}: {{ $valoresReferencia['deseable']['rango'] ?? 'Menor de 200 mg/dL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['limite_normal_alto']))
                            <div>{{ $valoresReferencia['limite_normal_alto']['label'] ?? 'Limite entre normal y alto' }}: {{ $valoresReferencia['limite_normal_alto']['rango'] ?? '200 a 239 mg/dL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['alto']))
                            <div>{{ $valoresReferencia['alto']['label'] ?? 'Alto' }}: {{ $valoresReferencia['alto']['rango'] ?? 'Mayor de 240 mg/dL' }}</div>
                        @endif
                    @else
                        <div>Deseable: Menor de 200 mg/dL</div>
                        <div>Limite entre normal y alto: 200 a 239 mg/dL</div>
                        <div>Alto: Mayor de 240 mg/dL</div>
                    @endif
                </td>
            </tr>
        </tbody>
