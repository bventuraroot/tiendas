...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Coombs Directo y Indirecto' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.coombs_directo_indirecto.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'coombs_indirecto', 'label' => 'Coombs Indirecto', 'unidad' => ''],
            ['key' => 'coombs_directo', 'label' => 'Coombs Directo', 'unidad' => ''],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">HEMATOLOGÍA E INMUNOHEMATOLOGÍA</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    if (!$r && $resultsByParam->isNotEmpty()) {
                        ...
                        foreach ($resultsByParam as $key => $result) {
                            ...
                            if (stripos($keyLower, 'indirecto') !== false && stripos($labelLower, 'indirecto') !== false) {
                                $r = $result;
                                break;
                            }
                            if (stripos($keyLower, 'directo') !== false && stripos($labelLower, 'directo') !== false) {
                                $r = $result;
                                break;
                            }
                        }
