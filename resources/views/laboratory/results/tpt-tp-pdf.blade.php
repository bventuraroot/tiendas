...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'TPT Y TP' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        ...
        $valRef = config('lab_exam_templates.tpt_tp.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'tpt', 'label' => 'Tiempo de Tromboplastina Parcial (TTP)', 'unidad' => 'segundos'],
            ['key' => 'tp', 'label' => 'Tiempo de protombina (TP)', 'unidad' => 'segundos'],
