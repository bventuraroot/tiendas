...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Colinesterasa' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'ENZIMAS' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'U/L');
            @endphp
            ...
            <tr>
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['general']))
                            <div>{{ $valoresReferencia['general']['rango'] ?? '5,320 a 12,920 U/L' }}</div>
                        @endif
                        @if(isset($valoresReferencia['embarazadas_anticonceptivos']))
                            <div>{{ $valoresReferencia['embarazadas_anticonceptivos']['label'] ?? 'Embarazadas o tomando Anticonceptivos' }}: {{ $valoresReferencia['embarazadas_anticonceptivos']['rango'] ?? '3,650 a 13,977 U/L' }}</div>
                        @endif
                    @else
                        <div>5,320 a 12,920 U/L</div>
                        <div>Embarazadas o tomando Anticonceptivos: 3,650 a 13,977 U/L</div>
                    @endif
                </td>
            </tr>
        </tbody>
