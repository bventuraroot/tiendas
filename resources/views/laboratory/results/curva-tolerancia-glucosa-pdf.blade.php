...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Curva de Tolerancia a la Glucosa' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        ...
        $valRef = config('lab_exam_templates.curva_tolerancia_glucosa.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'ayunas', 'label' => 'Glucosa en Ayunas', 'unidad' => 'mg/dL'],
            ['key' => '1h', 'label' => 'Glucosa 1 Hora después de la toma', 'unidad' => 'mg/dL'],
            ['key' => '2h', 'label' => 'Glucosa 2 Hora después de la toma', 'unidad' => 'mg/dL'],
            ['key' => '3h', 'label' => 'Glucosa 3 Hora después de la toma', 'unidad' => 'mg/dL'],
        ];

        $categoria = $valoresReferencia['categoria'] ?? ($exam->category->nombre ?? 'BIOQUIMICA CLINICA');
        $notaDextrosa = $valRef['nota_dextrosa'] ?? 'Se Administro 100 Gramos de Dextrosa.';
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">{{ strtoupper($categoria) }}</td>
            </tr>
            <tr>
                <td class="exam-subtitle" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">Curva de Tolerancia a la Glucosa</td>
            </tr>
            @foreach($params as $p)
                @php
