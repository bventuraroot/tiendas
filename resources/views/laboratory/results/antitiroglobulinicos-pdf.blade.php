...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Ac. AntitiroGlobulinicos (ATT)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'PRUEBAS ESPECIALIZADAS' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'UI/mL');
            @endphp
            
            <tr>
                <td class="exam-name">{{ $exam->nombre }}:</td>
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['adultos']))
                            <div class="reference-item">{{ $valoresReferencia['adultos']['label'] ?? 'Adultos' }} {{ $valoresReferencia['adultos']['rango'] ?? 'Hasta 115 UI/mL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['neonatos']))
                            <div class="reference-item">{{ $valoresReferencia['neonatos']['label'] ?? 'Neonatos' }} {{ $valoresReferencia['neonatos']['rango'] ?? 'Hasta 134 UI/mL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['6_dias_3_meses']))
                            <div class="reference-item">{{ $valoresReferencia['6_dias_3_meses']['label'] ?? '6 Días a 3 Meses' }} {{ $valoresReferencia['6_dias_3_meses']['rango'] ?? 'Hasta 146 UI/mL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['4_12_meses']))
                            <div class="reference-item">{{ $valoresReferencia['4_12_meses']['label'] ?? '4 a 12 Meses' }} {{ $valoresReferencia['4_12_meses']['rango'] ?? 'Hasta 130 UI/mL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['1_11_anos']))
                            <div class="reference-item">{{ $valoresReferencia['1_11_anos']['label'] ?? '1 a 11 Años' }} {{ $valoresReferencia['1_11_anos']['rango'] ?? 'Hasta 38 UI/mL' }}</div>
                        @endif
                        @if(isset($valoresReferencia['12_20_anos']))
                            <div class="reference-item">{{ $valoresReferencia['12_20_anos']['label'] ?? '12 a 20 Años' }} {{ $valoresReferencia['12_20_anos']['rango'] ?? 'Hasta 64 UI/mL' }}</div>
                        @endif
                    @else
                        <div class="reference-item">Adultos Hasta 115 UI/mL</div>
                        <div class="reference-item">Neonatos Hasta 134 UI/mL</div>
                        <div class="reference-item">6 Días a 3 Meses Hasta 146 UI/mL</div>
                        <div class="reference-item">4 a 12 Meses Hasta 130 UI/mL</div>
                        <div class="reference-item">1 a 11 Años Hasta 38 UI/mL</div>
                        <div class="reference-item">12 a 20 Años Hasta 64 UI/mL</div>
                    @endif
                </td>
            </tr>
        </tbody>
