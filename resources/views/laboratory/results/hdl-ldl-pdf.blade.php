...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Colesterol HDL Y LDL' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.hdl_ldl.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'ldl', 'label' => 'Colesterol de Baja Densidad (LDL)', 'unidad' => 'mg/dL'],
            ['key' => 'hdl', 'label' => 'Colesterol de Alta Densidad (HDL)', 'unidad' => 'mg/dL'],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">HDL Y LDL</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    // Buscar resultado por label exacto
                    $r = $resultsByParam->get($label);
                    // Si no se encuentra, buscar por coincidencia parcial (similar a otros PDFs)
                    if (!$r) {
                        foreach ($resultsByParam as $key => $result) {
                            if (stripos($key, 'ldl') !== false && stripos($label, 'ldl') !== false) {
                                $r = $result;
                                break;
                            } elseif (stripos($key, 'hdl') !== false && stripos($label, 'hdl') !== false) {
                                $r = $result;
                                break;
                            }
                        }
