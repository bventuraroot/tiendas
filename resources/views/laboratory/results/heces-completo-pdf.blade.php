...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'HECES COMPLETO' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.heces_completo.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'color', 'label' => 'Color', 'unidad' => ''],
            ['key' => 'consistencia', 'label' => 'Consistencia', 'unidad' => ''],
            ['key' => 'mucus', 'label' => 'Mucus', 'unidad' => ''],
            ['key' => 'restos_alimenticios_macroscopicos', 'label' => 'Restos alimenticios Macroscópicos', 'unidad' => ''],
            ['key' => 'restos_alimenticios_microscopicos', 'label' => 'Restos alimenticios Microscópicos', 'unidad' => ''],
            ['key' => 'metazoarios', 'label' => 'Metazoarios', 'unidad' => ''],
            ['key' => 'protozoarios_quistes', 'label' => 'Protozoarios Quistes', 'unidad' => ''],
            ['key' => 'protozoarios_activos', 'label' => 'Protozoarios Activos', 'unidad' => ''],
            ['key' => 'hematies', 'label' => 'Hematíes', 'unidad' => ''],
            ['key' => 'leucocitos', 'label' => 'Leucocitos', 'unidad' => ''],
            ['key' => 'levaduras', 'label' => 'Levaduras', 'unidad' => ''],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">HECES COMPLETO</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    if (!$r && $resultsByParam->isNotEmpty()) {
                        foreach ($resultsByParam as $key => $result) {
                            ...
                            // Buscar por palabras clave del parámetro
                            $keywords = explode(' ', $labelLower);
                            $matchCount = 0;
                            foreach ($keywords as $keyword) {
                                if (strlen($keyword) > 3 && stripos($keyLower, $keyword) !== false) {
                                    $matchCount++;
                                }
                            }
                            if ($matchCount >= 2 || stripos($keyLower, $labelLower) !== false) {
                                $r = $result;
                                break;
                            }
                        }
                        ...
                        if (!$r && $resultsByParam->isNotEmpty()) {
                            foreach ($resultsByParam as $key => $result) {
                                $keyLower = strtolower($key);
                                $labelLower = strtolower($label);
                                if (stripos($keyLower, substr($labelLower, 0, 5)) !== false) {
                                    $r = $result;
                                    break;
                                }
                            }
