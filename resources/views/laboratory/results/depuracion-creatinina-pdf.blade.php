...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Depuración de Creatinina' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <!-- Tabla de Resultados (Depuración de Creatinina) -->
    @php
        $ref = $valoresReferencia ?? [];
        $valRef = config('lab_exam_templates.depuracion_creatinina.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'creatinina_suero', 'label' => 'Creatinina en Suero', 'labelAlt' => 'Creatinina en Suero', 'unidad' => 'mg/dL'],
            ['key' => 'creatinina_orina', 'label' => 'Creatinina en Orina', 'labelAlt' => 'Creatinina en Orina', 'unidad' => 'mg/dL'],
            ['key' => 'depuracion', 'label' => 'Depuración de Creatinina', 'labelAlt' => 'Depuración de Creatinina', 'unidad' => 'mL/Minuto'],
            ['key' => 'volumen_orina', 'label' => 'Volumen de Orina', 'labelAlt' => 'Volumen de Orina (mL/24 Horas)', 'unidad' => 'mL/24 Horas'],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">DEPURACION DE CREATININA</td>
            </tr>
            @foreach($params as $p)
                @php
                    $r = $resultsByParam->get($p['label']) ?? $resultsByParam->get($p['labelAlt']);
                    $resVal = $r ? $r->resultado : '';
                    $u = $r && $r->unidad_medida ? str_replace(['μg', 'μmol', 'μL'], ['ug', 'umol', 'uL'], $r->unidad_medida) : ($p['unidad'] ?? '');
                    ...
                    $displayLabel = $p['key'] === 'volumen_orina' ? 'Volumen de Orina:' : $p['label'] . ':';
                ...
                @endphp
                <tr>
                    <td class="exam-name">{{ $displayLabel }}</td>
                    <td>
                        @if($resVal !== '')
                            {{ $resVal }} {{ $u }}
