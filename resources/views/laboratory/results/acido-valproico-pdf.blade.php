...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Ácido Valpróico' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'DROGAS TERAPEUTICAS' }}</td>
                <td></td>
                <td></td>
            </tr>
            <tr>
                ...
                <td style="text-align: center;">
                    @php
                        ...
                        $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'ug/mL');
                    @endphp
                    {{ $resultValue }} {{ $unit }}
                </td>
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['terapeutico']))
                            Terapéutico: {{ $valoresReferencia['terapeutico']['rango'] ?? '50 a 100 ug/mL' }}<br>
                        @endif
                        @if(isset($valoresReferencia['toxico']))
                            Tóxico: {{ $valoresReferencia['toxico']['rango'] ?? 'Mayor de 100 ug/mL' }}
                        @endif
                    @else
                        Terapéutico: 50 a 100 ug/mL<br>
...
<!-- Validación -->
    <div class="validation-section">
        @php
            // Mostrar como "Validado por" al médico que autorizó/ordenó la prueba
