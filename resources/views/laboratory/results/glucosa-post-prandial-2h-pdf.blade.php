...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Glucosa post-pandrial 2 horas' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.glucosa_post_pandrial_2h.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'ayunas', 'label' => 'Glucosa en Ayunas', 'unidad' => 'mg/dL'],
            ['key' => 'post_prandial', 'label' => 'Glucosa Post-Prandial', 'unidad' => 'mg/dL'],
        ];
    @endphp
