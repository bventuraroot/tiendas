...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'ANTIGENOS FEBRILES' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.antigenos_febriles.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'salmonella_paratyphi_ah', 'label' => 'SALMONELLA PARATYPHI AH', 'unidad' => ''],
            ['key' => 'salmonella_paratyphi_bh', 'label' => 'SALMONELLA PARATYPHI BH', 'unidad' => ''],
            ['key' => 'salmonella_typhi_h', 'label' => 'SALMONELLA TYPHI H', 'unidad' => ''],
            ['key' => 'salmonella_typhi_o', 'label' => 'SALMONELLA TYPHI O', 'unidad' => ''],
            ['key' => 'brucella_abortus', 'label' => 'BRUCELLA ABORTUS', 'unidad' => ''],
            ['key' => 'proteus_ox19', 'label' => 'PROTEUS OX19', 'unidad' => ''],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">ANTIGENOS FEBRILES</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    if (!$r && $resultsByParam->isNotEmpty()) {
                        ...
                        foreach ($resultsByParam as $key => $result) {
                            ...
                            // Buscar por palabras clave del label
                            $labelWords = explode(' ', $labelLower);
                            $keyWords = explode(' ', $keyLower);
                            $matches = 0;
                            ...
                            // Búsqueda específica por parámetro
                            if (stripos($keyLower, 'paratyphi') !== false && stripos($labelLower, 'paratyphi') !== false) {
                                if (stripos($keyLower, 'ah') !== false && stripos($labelLower, 'ah') !== false) {
                                    $r = $result;
                                    break;
                                }
                                if (stripos($keyLower, 'bh') !== false && stripos($labelLower, 'bh') !== false) {
                                    $r = $result;
                                    break;
                                }
                            }
                            if (stripos($keyLower, 'typhi') !== false && stripos($labelLower, 'typhi') !== false) {
                                if (stripos($keyLower, ' h') !== false && stripos($labelLower, ' h') !== false) {
                                    $r = $result;
                                    break;
                                }
                                if (stripos($keyLower, ' o') !== false && stripos($labelLower, ' o') !== false) {
                                    $r = $result;
                                    break;
                                }
                            }
                            if (stripos($keyLower, 'brucella') !== false && stripos($labelLower, 'brucella') !== false) {
                                $r = $result;
                                break;
                            }
                            if (stripos($keyLower, 'proteus') !== false && stripos($labelLower, 'proteus') !== false) {
                                $r = $result;
                                break;
                            }
                        }
                ...
                <tr>
                    ...
                    <td>
                        @if($resVal !== '')
                            {{ strtoupper($resVal) }} @if($u) {{ $u }} @endif
                        @endif
                    </td>
