...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'FROTIS DE SANGRE PERIFÉRICA' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.frotis_sangre_periferica.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'serie_roja', 'label' => 'Serie Roja', 'unidad' => ''],
            ['key' => 'serie_blanca', 'label' => 'Serie Blanca', 'unidad' => ''],
            ['key' => 'serie_plaquetaria', 'label' => 'Serie Plaquetaria', 'unidad' => ''],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">FROTIS DE SANGRE PERIFÉRICA</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    if (!$r && $resultsByParam->isNotEmpty()) {
                        ...
                        foreach ($resultsByParam as $key => $result) {
                            ...
                            if (stripos($keyLower, 'serie') !== false && stripos($labelLower, 'serie') !== false) {
                                if (stripos($keyLower, 'roja') !== false && stripos($labelLower, 'roja') !== false) {
                                    $r = $result;
                                    break;
                                }
                                if (stripos($keyLower, 'blanca') !== false && stripos($labelLower, 'blanca') !== false) {
                                    $r = $result;
                                    break;
                                }
                                if (stripos($keyLower, 'plaquetaria') !== false && stripos($labelLower, 'plaquetaria') !== false) {
                                    $r = $result;
                                    break;
                                }
                            }
