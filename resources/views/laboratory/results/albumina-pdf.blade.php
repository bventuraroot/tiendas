...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Albúmina' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'PRUEBAS RENALES' }}</td>
                <td></td>
                <td></td>
            </tr>
            <tr>
                ...
                <td style="text-align: center;">
                    @php
                        ...
                        $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'g/dL');
                    @endphp
                    {{ $resultValue }} {{ $unit }}
                </td>
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['adultos']))
                            {{ $valoresReferencia['adultos']['label'] ?? 'Adultos' }}: {{ $valoresReferencia['adultos']['rango'] ?? '3.5 a 5.2 g/dL' }}<br>
                        @endif
                        @if(isset($valoresReferencia['neonatos_0_4_dias']))
                            {{ $valoresReferencia['neonatos_0_4_dias']['label'] ?? 'Neonatos 0 a 4 días' }}: {{ $valoresReferencia['neonatos_0_4_dias']['rango'] ?? '2.8 a 4.4 g/dL' }}<br>
                        @endif
                        @if(isset($valoresReferencia['ninos_4_dias_14_anos']))
                            {{ $valoresReferencia['ninos_4_dias_14_anos']['label'] ?? 'Niños 4 días a 14 años' }}: {{ $valoresReferencia['ninos_4_dias_14_anos']['rango'] ?? '3.8 a 5.4 g/dL' }}<br>
                        @endif
                        @if(isset($valoresReferencia['ninos_14_18_anos']))
                            {{ $valoresReferencia['ninos_14_18_anos']['label'] ?? 'Niños 14 años a 18 años' }}: {{ $valoresReferencia['ninos_14_18_anos']['rango'] ?? '3.2 a 4.5 g/dL' }}
                        @endif
                    @else
                        Adultos: 3.5 a 5.2 g/dL<br>
                        Neonatos 0 a 4 días: 2.8 a 4.4 g/dL<br>
                        Niños 4 días a 14 años: 3.8 a 5.4 g/dL<br>
