...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'UROCULTIVO (Cultivo de Orina)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.urocultivo.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'recuento_bacteriano', 'label' => 'Recuento Bacteriano', 'unidad' => 'UFC/mL'],
            ['key' => 'susceptible', 'label' => 'Susceptible', 'unidad' => ''],
            ['key' => 'intermedio', 'label' => 'Intermedio', 'unidad' => ''],
            ['key' => 'resistente', 'label' => 'Resistente', 'unidad' => ''],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">UROCULTIVO (Cultivo de Orina)</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    if (!$r && $resultsByParam->isNotEmpty()) {
                        foreach ($resultsByParam as $key => $result) {
                            ...
                            if ($matchCount >= 1 || stripos($keyLower, $labelLower) !== false) {
                                $r = $result;
                                break;
                            }
                        }
