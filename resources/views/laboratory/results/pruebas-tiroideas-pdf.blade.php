...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Pruebas Tiroideas' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'PRUEBAS TIROIDEAS' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                // Obtener los tres resultados (compatibilidad con ambos formatos)
                $resultadoTsh = $results->firstWhere('parametro', 'Hormona Estimulante de Tiroides (TSH)');
                $resultadoT4 = $results->firstWhere('parametro', 'Tiroxina (T4)') 
                    ?? $results->firstWhere('parametro', 'T4 Libre (FT4)');
                $resultadoT3 = $results->firstWhere('parametro', 'Triyodotironina (T3)')
                    ?? $results->firstWhere('parametro', 'T3 Libre (FT3)');
            @endphp
            
            <!-- Fila para T3 -->
            <tr>
                <td class="exam-name">Triyodotironina (T3):</td>
                <td style="text-align: center;">
                    @if($resultadoT3)
                        {{ $resultadoT3->resultado }} {{ $resultadoT3->unidad_medida ?? 'ng/mL' }}
                    @endif
                </td>
                <td class="reference-range">
                    @if($resultadoT3 && $resultadoT3->valor_referencia)
                        {{ $resultadoT3->valor_referencia }}
                    @else
                        0.5 - 5.0 ng/mL
                    @endif
                </td>
            ...
            <!-- Fila para T4 -->
            <tr>
                <td class="exam-name">Tiroxina (T4):</td>
                <td style="text-align: center;">
                    @if($resultadoT4)
                        {{ $resultadoT4->resultado }} {{ $resultadoT4->unidad_medida ?? 'nmol/L' }}
                    @endif
                </td>
                <td class="reference-range">
                    @if($resultadoT4 && $resultadoT4->valor_referencia)
                        {{ $resultadoT4->valor_referencia }}
                    @else
                        60 - 120 nmol/L
                    @endif
                </td>
            ...
            <!-- Fila para TSH -->
            <tr>
                <td class="exam-name">Hormona Estimulante de Tiroides (TSH):</td>
                <td style="text-align: center;">
                    @if($resultadoTsh)
                        {{ $resultadoTsh->resultado }} {{ $resultadoTsh->unidad_medida ?? 'uUI/mL' }}
                    @endif
                </td>
                <td class="reference-range">
                    @if($resultadoTsh && $resultadoTsh->valor_referencia)
                        {{ $resultadoTsh->valor_referencia }}
                    @else
                        0.4 - 4.0 uUI/mL
                    @endif
                </td>
