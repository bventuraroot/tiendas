...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Citomegalovirus (CMV - IgM)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'QUIMICA E INMUNOLOGIA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                ...
                $unit = $valoresReferencia['unidad_medida'] ?? ($result && $result->unidad_medida ? $result->unidad_medida : 'COI');
            @endphp
            ...
            <tr>
                ...
                <td class="reference-range">
                    @if($valoresReferencia)
                        @if(isset($valoresReferencia['no_reactivo']))
                            <div>{{ $valoresReferencia['no_reactivo']['label'] ?? 'No Reactivo' }}: {{ $valoresReferencia['no_reactivo']['rango'] ?? 'Menor a 0.7 COI' }}</div>
                        @endif
                        @if(isset($valoresReferencia['indeterminado']))
                            <div>{{ $valoresReferencia['indeterminado']['label'] ?? 'Indeterminado' }}: {{ $valoresReferencia['indeterminado']['rango'] ?? '0.7 a 1.0 COI' }}</div>
                        @endif
                        @if(isset($valoresReferencia['reactivo']))
                            <div>{{ $valoresReferencia['reactivo']['label'] ?? 'Reactivo' }}: {{ $valoresReferencia['reactivo']['rango'] ?? 'Mayor a 1.0 COI' }}</div>
                        @endif
                    @else
                        <div>No Reactivo: Menor a 0.7 COI</div>
                        <div>Indeterminado: 0.7 a 1.0 COI</div>
                        <div>Reactivo: Mayor a 1.0 COI</div>
                    @endif
                </td>
            </tr>
        </tbody>
