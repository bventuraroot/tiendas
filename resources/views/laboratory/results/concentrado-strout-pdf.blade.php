...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Concentrado de strout' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.concentrado_strout.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'resultado', 'label' => 'Resultado', 'unidad' => ''],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">INMUNOHEMTOLOGIA</td>
            </tr>
            @foreach($params as $p)
                @php
                    ...
                    // Si no se encuentra por label exacto, buscar por coincidencia parcial o por cualquier resultado
                    if (!$r && $resultsByParam->isNotEmpty()) {
                        // Buscar por "resultado" en el nombre del parámetro
                        foreach ($resultsByParam as $key => $result) {
                            $keyLower = strtolower($key);
                            if (stripos($keyLower, 'resultado') !== false || stripos($keyLower, 'concentrado') !== false || stripos($keyLower, 'strout') !== false) {
                                $r = $result;
                                break;
                            }
                        }
                        // Si aún no se encuentra, tomar el primer resultado disponible
                        if (!$r && $resultsByParam->isNotEmpty()) {
                            $r = $resultsByParam->first();
                        }
                    }
