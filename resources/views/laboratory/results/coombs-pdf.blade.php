...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Coombs' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category">{{ $valoresReferencia['categoria'] ?? 'HEMATOLOGÍA E INMUNOHEMATOLOGÍA' }}</td>
                <td></td>
                <td></td>
            </tr>
            @php
                // Obtener los dos resultados
                $resultadoIndirecto = $results->firstWhere('parametro', 'Coombs Indirecto');
                $resultadoDirecto = $results->firstWhere('parametro', 'Coombs Directo');
            @endphp
            
            <!-- Coombs Indirecto -->
            <tr>
                <td class="exam-name">Coombs Indirecto:</td>
                <td style="text-align: center;">
                    @if($resultadoIndirecto)
                        {{ $resultadoIndirecto->resultado }}
                    @endif
                </td>
            ...
            <!-- Coombs Directo -->
            <tr>
                <td class="exam-name">Coombs Directo:</td>
                <td style="text-align: center;">
                    @if($resultadoDirecto)
                        {{ $resultadoDirecto->resultado }}
                    @endif
                </td>
                <td class="reference-range">
                    Negativo / Positivo
