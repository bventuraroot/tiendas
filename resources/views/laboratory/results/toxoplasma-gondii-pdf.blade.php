...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Toxoplasma gondii IgM IgG' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        $valRefConfig = config('lab_exam_templates.toxoplasma_gondii.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'igg', 'label' => 'Toxoplasma gondii IgG', 'unidad' => 'UI/mL'],
            ['key' => 'igm', 'label' => 'Toxoplasma gondii IgM', 'unidad' => 'COI'],
        ];
    @endphp
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td class="exam-category" colspan="3" style="padding-top: 1%; padding-bottom: 1%;">TORCH</td>
            </tr>
            @foreach($params as $p)
                @php
                ...
                <tr>
                    ...
                    <td class="reference-range">
                        {!! nl2br(e($vr)) !!}
                        @if($nota)
                            <div class="nota-recomendacion">{{ $nota }}</div>
                        @endif
                    </td>
                </tr>
            @endforeach
