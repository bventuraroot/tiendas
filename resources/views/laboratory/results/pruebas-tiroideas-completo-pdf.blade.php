...
<head>
    <meta charset="UTF-8">
    <title>Resultado de Examen - {{ $exam->nombre ?? 'Pruebas Tiroideas (TSH, T4 Libre, T3 Libre)' }}</title>
    <style>
        @page {
            margin: 1.5cm;
            size: Letter;
...
<body>
    ...
    @php
        ...
        $valRef = config('lab_exam_templates.pruebas_tiroideas_completo.valores_referencia', []);
        $resultsByParam = $results->keyBy('parametro');

        $params = [
            ['key' => 'tsh', 'label' => 'Hormona Estimulante de Tiroides (TSH)', 'unidad' => 'uUI/mL'],
            ['key' => 'ft4', 'label' => 'T4 Libre (FT4)', 'unidad' => 'ng/dl'],
            ['key' => 'ft3', 'label' => 'T3 Libre (FT3)', 'unidad' => 'pg/ml'],
        ];
